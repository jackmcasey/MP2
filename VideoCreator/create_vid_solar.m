...
% Function to plot and record an interactive plot of the solar system using
...
% Includes record switch that allows a custom video with variable number of
% days in each frame at a rate of 10 frames per second
...
VideoObject = VideoWriter('solarsystem.avi');       % open object for video
VideoObject.FrameRate = 10;                         % define frame rate
open(VideoObject);
...
toggle = uiswitch(uifig,'Position',[40 275 120 15],'Items',{'Stop','Play'},'ItemsData',[0 1]);%play switch    
rec = uiswitch(uifig,'Position',[40 305 120 15],'Items',{'Off','Record'},'ItemsData',[0 1]);% record button
...
sldt.ValueChangedFcn = @(sldt,event) solarsyscallback(SIM_TIME,sys,data,solarsystem,sldt,sldz,speed,toggle,rec,select,txt,VideoObject);
sldz.ValueChangedFcn = @(sldz,event) solarsyscallback(SIM_TIME,sys,data,solarsystem,sldt,sldz,speed,toggle,rec,select,txt,VideoObject);
speed.ValueChangedFcn = @(speed,event) solarsyscallback(SIM_TIME,sys,data,solarsystem,sldt,sldz,speed,toggle,rec,select,txt,VideoObject);
toggle.ValueChangedFcn = @(toggle,event) solarsyscallback(SIM_TIME,sys,data,solarsystem,sldt,sldz,speed,toggle,rec,select,txt,VideoObject);
rec.ValueChangedFcn = @(rec,event) solarsyscallback(SIM_TIME,sys,data,solarsystem,sldt,sldz,speed,toggle,rec,select,txt,VideoObject);
select.SelectionChangedFcn = @(select,event) solarsyscallback(SIM_TIME,sys,data,solarsystem,sldt,sldz,speed,toggle,rec,select,txt,VideoObject);
...
reset = uibutton(uifig,'Position',[175 35 100 20],'Text','Reset Axis','ButtonPushedFcn',@(reset,event) resetaxis(gca,sldz,sldt,toggle,select,solarsystem,data,txt)) ;
% button to close simulation
ex = uibutton(uifig,'Position',[300 35 100 20],'Text','Close','ButtonPushedFcn',@(ex,event) exfig(uifig)) ;
% switch to finish recording
fin = uibutton(uifig,'state','Position',[425 35 100 20],'Text','Finish Record');
waitfor(fin,'Value')        % video closes after button press
close(VideoObject)
